...
//       You can use 1 SparkfunMP3 Trigger either with or without a WTV020 module
...
int busyPin = A0;  // The pin number of the busy pin.
int clockPin2 = 6;  // The pin number of the clock pin.
int dataPin2 = 9;  // The pin number of the data pin.
int busyPin2 = 2;  // The pin number of the busy pin.
//#if (AUDIO1==2)
WtvPadawan wtv(clockPin,dataPin,busyPin);
//#else
MP3Trigger trigger;
//#endif
...
//#if (AUDIO2==2)
WtvPadawan wtv2(clockPin2,dataPin2,busyPin2);
//#endif
//////////////////////////////////////////////////////////////////
...
byte wtvVol = 6; // 7 = full volume, 0 off
byte trigVol = 30;
byte wtvVol2 = 5; // 7 = full volume, 0 off
byte trigVol2 = 30;
...
void setup(){
  ...
  wtv2.setVolume(wtvVol2);
  wtv.setVolume(wtvVol);
  #if defined(SYRENSIMPLE)
  SyR.motor(0);            
  #else
  SyR.autobaud(); 
...
while(attempts<25) { //loop around until a PS2 controller is found or we've attempted 25 times
 ...
 wtv2.setVolume(wtvVol2);//Yes, this is a repeat, but somtimes the first one doesn't take :p
 wtv.setVolume(wtvVol);
 }
...
////volume controll
 if(ps2x.ButtonPressed(PSB_PAD_UP)) 
 {
  if(ps2x.Button(PSB_R1))
   ...
   {wtvVol++;
    trigVol-=15;
    setVol1(wtvVol,trigVol);}// volume up
   }  
 }
  ...
  if(ps2x.Button(PSB_R1))
   ...
   {wtvVol--;
    trigVol+=15;
    setVol1(wtvVol, trigVol);}// volume down   
   } 
 }
 if(ps2x.ButtonPressed(PSB_PAD_RIGHT)) 
 {
  if(ps2x.Button(PSB_R1))
   ...
   {wtvVol2++;
    trigVol2-=15;
   setVol2(wtvVol2, trigVol2);}// volume up
   }  
 }
  ...
  if(ps2x.Button(PSB_R1))
   ...
   {wtvVol2--;
    trigVol2+=15;
   setVol2(wtvVol2, trigVol2);}// volume down   
   } 
 }
 ...
 {
   if (AUDIO1 == 2)
     {wtv.stopVoice(); domeData.dsp = 0; ET.sendData();}
   if (AUDIO2 == 2)  
     {wtv2.stopVoice(); domeData.dsp = 0; ET.sendData();}
   if (AUDIO1 == 1)  
...
void playSound(int soundNumber) {
  if (AUDIO1 == 1)
    ...
    wtv.asyncPlayVoice(soundNumber);
  //endif  
}
...
void playMusic(int musicNumber) {
  if (AUDIO2 == 1)
    ...
    wtv2.asyncPlayVoice(musicNumber);
  else  
...
void setVol1(byte wtvVol1, byte trigVol1) {
  if (AUDIO1 == 2)
     wtv.setVolume(wtvVol1);
   else
     trigger.setVolume(trigVol1);}
     
void setVol2(byte  wtvVol2, byte trigVol2) {
  if (AUDIO2 == 2)
     wtv2.setVolume( wtvVol2);
   else
